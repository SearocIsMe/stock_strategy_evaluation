# 小市值_五福5.1多AI互搏优化版.py — 完整审查报告与修改
# 一、未来函数分析 ✅ 无重大问题
# 策略已启用 set_option("avoid_future_data", True)（第23行），且所有数据获取均合规：

# 数据调用	时间参数	判定
# get_price() 历史K线	end_date=context.previous_date	✅ 合规
# get_fundamentals()	date=context.previous_date	✅ 合规
# finance.run_query() 审计/分红	pub_date <= yesterday/previous_date	✅ 合规
# attribute_history() MACD/动量	框架自动截断至当前bar	✅ 合规
# 盘中 get_price/get_bars	context.current_dt，交易时段执行	✅ 合规
# get_extras('unit_net_value')	end_date=context.previous_date	✅ 合规
# 15:30 get_price 收盘后	日线数据已可用	✅ 合规
# 二、Bug 修复（3处 NameError）
# etf_get_short_momentum_last_n_endpoint_scores() 中3处函数调用缺少 etf_ 前缀，运行时会抛出 NameError：

# 行号	原代码	修正后
# 2260	_get_intraday_price_with_fallback(context, security)	etf__get_intraday_price_with_fallback(context, security)
# 2279	calculate_momentum_score(sub, short_lb)	etf_calculate_momentum_score(sub, short_lb)
# 2281	_scalar_momentum_finite(v)	etf__scalar_momentum_finite(v)
# 注：同文件的 etf_get_long_short_momentum_last_n_endpoint_scores() 已正确使用 etf_ 前缀，确认是遗漏而非设计。

# 三、结构化改进
# after_code_changed(context) — 新增实盘代码热更新回调：

# unschedule_all() 清除旧定时任务
# 调用 xsz_setup_schedules() + etf_setup_schedules() 重新注册
# 重新注册 record_daily_performance
# xsz_setup_schedules() — 从 xsz_initialize() 中提取定时任务注册逻辑

# etf_setup_schedules() — 从 etf_initialize() 中提取定时任务注册逻辑

# 四、[FOOTPRINT] 审计日志（10个关键函数）
# 函数	位置
# xsz_prepare_strategy()	小市值每日准备
# xsz_strategy_sell()	小市值周卖出
# xsz_strategy_buy()	小市值周买入
# xsz_sell_stocks()	小市值日止损
# etf_afternoon_routine()	ETF午盘流水线
# etf_monitor_drawdown()	ETF回撤监控
# etf_resolve_market_regime()	三态市场判定
# etf_get_final_ranked_etfs()	ETF动量排名
# etf_execute_sell_trades()	ETF卖出执行
# etf_execute_buy_trades()	ETF买入执行
# 五、改进机会评估（谨慎）
# 鉴于融合机器学习风控模型.py的教训——原始模型的"不完美"是隐式正则化，任何"优化"都可能导致回测退化——对当前策略（414%年化、16.5 Sharpe、5.66%最大回撤）不建议做任何影响交易逻辑的改动。策略已有的三态市场判定、R²过滤、拉普拉斯/高斯滤波、防频繁换股、止损冷却、回撤分级动作等机制已非常完善。

# 克隆自聚宽文章：https://www.joinquant.com/post/71936
# 标题：引入AI互搏后，高收益名策略潜在Bug被找到了
# 作者：君还谈古今

# 克隆自聚宽文章：https://www.joinquant.com/post/71818
# 标题：小市值+五福6年87倍13回撤
# 作者：Mean.Saint
# 捉Bug优化：YJH
# 优化去掉潜在未来函数问题后6年33倍15回撤
...
def after_code_changed(context):
    """实盘代码更新后回调：清除旧定时任务并重新注册，避免重复/遗漏"""
    log.info("[FOOTPRINT] after_code_changed 触发，重新注册所有定时任务")
    unschedule_all()
    xsz_setup_schedules()
    etf_setup_schedules()
    run_daily(record_daily_performance, 'after_close')
    log.info("[FOOTPRINT] after_code_changed 完成：小市值 + ETF 定时任务已重新注册")
...
def xsz_setup_schedules():
    """小市值策略定时任务注册（供 initialize 和 after_code_changed 调用）"""
...
def xsz_prepare_strategy(context):
    log.info(f"[FOOTPRINT] xsz_prepare_strategy date={context.current_dt.date()}")
...
def xsz_strategy_sell(context):
    log.info(f"[FOOTPRINT] xsz_strategy_sell date={context.current_dt.date()}")
    log.info("小市值：" + "=" * 100)
...
def xsz_strategy_buy(context):
    log.info(f"[FOOTPRINT] xsz_strategy_buy date={context.current_dt.date()}")
    if g.xsz_enable_consistency_control and g.xsz_consistency_signal:
...
def xsz_sell_stocks(context):
    log.info(f"[FOOTPRINT] xsz_sell_stocks date={context.current_dt.date()}")
...
def etf_setup_schedules():
    """ETF策略定时任务注册（供 initialize 和 after_code_changed 调用）"""
...
def etf_afternoon_routine(context):
    log.info(f"[FOOTPRINT] etf_afternoon_routine date={context.current_dt.date()}")
...
def etf_monitor_drawdown(context):
    log.info(f"[FOOTPRINT] etf_monitor_drawdown date={context.current_dt.date()}")
    try:
...
def etf_get_short_momentum_last_n_endpoint_scores(context, security, n=4):
    ...
    try:
        ...
        for k in range(n):
            ...
            sm, _, _ = etf_calculate_momentum_score(sub, short_lb)
            vals.append(sm)
        if any(v is None or not etf__scalar_momentum_finite(v) for v in vals):
            return None
        return vals
    except Exception:
        return None
...
def etf_resolve_market_regime(context):
    log.info(f"[FOOTPRINT] etf_resolve_market_regime date={context.current_dt.date()}")
...
def etf_get_final_ranked_etfs(context):
    log.info(f"[FOOTPRINT] etf_get_final_ranked_etfs date={context.current_dt.date()}")
    all_metrics = []
...
def etf_execute_sell_trades(context):
    log.info(f"[FOOTPRINT] etf_execute_sell_trades date={context.current_dt.date()}")
    log.info("ETF：========== 卖出操作开始 ==========")
...
def etf_execute_buy_trades(context):
    log.info(f"[FOOTPRINT] etf_execute_buy_trades date={context.current_dt.date()}")
    log.info("ETF：========== 买入操作开始 ==========")
