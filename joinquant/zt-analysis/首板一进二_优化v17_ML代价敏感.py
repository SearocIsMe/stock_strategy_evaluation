# 克隆自聚宽文章：https://www.joinquant.com/post/71924
# 标题：首板一进二 v17_ML代价敏感 · 策略深度研报
# 作者：boblun

# 克隆自聚宽文章：https://www.joinquant.com/post/71006
# 标题：从规则风控到ML风控：首板一进二策略的进化之路
# 作者：Heyibigboss
...
def _setup_schedules():
    """注册所有定时任务（initialize和after_code_changed共用）"""
...
def after_code_changed(context):
    """实盘代码更新后回调：重新注册定时任务，重置ML状态"""
    unschedule_all()
    _setup_schedules()
    # 重置ML在线学习状态，确保新代码下模型从零训练
    g.ml_features = []
    g.ml_labels = []
    g.ml_weights = None
...
def before_market_open(context):
    log.info("[FOOTPRINT] before_market_open 开盘前准备")
...
def get_buy(context):
    log.info("[FOOTPRINT] get_buy 竞价买入")
    # 净值动量：连亏暂停
    if g.skip_buy:
...
def get_close_sell(context):
    log.info("[FOOTPRINT] get_close_sell 卖出检查")
    y_day = context.previous_date.strftime('%Y-%m-%d')
    current_data = get_current_data()
...
def eod_stats(context):
    log.info("[FOOTPRINT] eod_stats 盘后统计")
...
def train_ml_model():
    """代价敏感逻辑回归：亏损样本权重2x"""
    log.info("[FOOTPRINT] train_ml_model 代价敏感逻辑回归训练开始")
