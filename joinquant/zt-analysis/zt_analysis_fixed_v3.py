...
CONFIG = {
    ...
    'top_k': 3,                          # 输出 Top K 股票
    'enable_jq_field_fill': True,          # CSV 缺失字段时，是否调用 JoinQuant API 补齐
    'score_weights': {
        'price_strength': 30,
        'trend_structure': 20,
        'volume': 20,
...
def _ensure_time_column(price_df):
    """兼容 JoinQuant get_price(panel=False) 的两种返回格式：time/date 列或日期索引。"""
    if price_df is None or price_df.empty:
        return price_df

    df = price_df.copy()

    # JoinQuant 在不同环境/版本中可能返回 time 列、date 列，或直接把日期放在 index。
    if 'time' in df.columns:
        ...
        df = df.reset_index()
        # reset_index 后第一列通常是 index/date/time。
...
def _jq_code_candidates(jq_code):
    """为可能的交易所后缀生成候选，避免 920/8/4 开头股票因后缀误判直接失败。"""
    jq_code = str(jq_code).strip().upper()
    base = jq_code.split('.')[0].zfill(6)
    ...
    # A 股常规后缀；北交所不同数据源/环境可能有不同支持，逐个尝试，失败则跳过。
    ...
    # 兼容部分环境可能存在的北交所后缀；如果 JoinQuant 不支持，会被 try/except 自动跳过。
    add(f'{base}.BJ')
    add(f'{base}.XBE')
    return candidates


def _get_price_forward_compatible(jq_code, zt_date_str, n_days):
    """获取涨停日及之后若干交易日行情；兼容 JoinQuant 日期列/索引差异和代码后缀误判。"""
    start_date = pd.to_datetime(zt_date_str)
    # 用较宽的自然日窗口覆盖 5 个交易日，遇到长假也尽量够用。
    end_date = (start_date + pd.Timedelta(days=max(30, n_days * 6))).strftime('%Y-%m-%d')

    last_error = None
    for code in _jq_code_candidates(jq_code):
        try:
            price_df = get_price(
                code,
            ...
            price_df = _ensure_time_column(price_df)
    ...
    if last_error is not None:
        raise last_error
    return jq_code, pd.DataFrame()



# ============================================================================
# JoinQuant 字段补全：CSV 有效值优先；CSV 未提供/为空时才补齐
# ============================================================================
PRICE_FIELD_MAP_JQ = {
    'open': 'open',
    'high': 'high',
...
# get_fundamentals 可补齐的常用估值/基本面字段。
# 注意：不同 JoinQuant 环境字段名可能略有差异，所以后续读取时全部做了安全判断。
FUNDAMENTAL_FIELD_CANDIDATES = {
    ...
    'float_shares': ['circulating_cap'],
    'float_market_cap': ['circulating_market_cap'],
    'eps': ['eps'],
    'roe': ['roe'],
    'roa': ['roa'],
    'revenue_yoy': ['inc_revenue_year_on_year'],
    'profit_yoy': ['inc_net_profit_year_on_year'],
    'gross_margin': ['gross_profit_margin'],
}


def _is_missing_value(value):
    """判断单个值是否缺失，兼容 '', '--', NaN 等行情软件常见空值。"""
    if value is None:
        return True
    try:
        if pd.isna(value):
        ...
        pass
    if isinstance(value, str) and value.strip() in ('', '--', 'nan', 'None', 'NaN'):
        return True
    return False


def _ensure_columns(df, columns):
    """确保 DataFrame 存在这些列；不存在则创建为 NaN。"""
    for col in columns:
...
def _fill_if_missing(df, idx, col, value):
    """仅当目标单元格缺失时写入，不覆盖 CSV 原有有效值。"""
    if col not in df.columns:
        df[col] = np.nan
    if _is_missing_value(df.at[idx, col]) and not _is_missing_value(value):
        df.at[idx, col] = value
        return True
    return False


def _get_single_day_price_safe(jq_code, date_str):
    """获取某只股票某一天附近的日线数据，兼容代码后缀和 time/index 差异。"""
    last_error = None
    for code in _jq_code_candidates(jq_code):
        try:
            price_df = get_price(
...
def _build_fundamentals_query(code_list):
    """构建 JoinQuant fundamentals 查询；不同环境字段缺失时自动降级。"""
    cols = []

    def add_col(obj, attr):
        try:
    ...
    # valuation 字段
    for attr in ['code', 'pe_ratio', 'pb_ratio', 'ps_ratio', 'capitalization',
                 'market_cap', 'circulating_cap', 'circulating_market_cap']:
        add_col(valuation, attr)

    # indicator 字段
    for attr in ['eps', 'roe', 'roa', 'inc_revenue_year_on_year',
                 'inc_net_profit_year_on_year', 'gross_profit_margin']:
        add_col(indicator, attr)

    if not cols:
        return None

    try:
        return query(*cols).filter(valuation.code.in_(code_list))
    except Exception:
        return None


def _get_fundamentals_safe(code_list, date_str=None):
    """批量获取估值/基本面数据。失败时返回空表，不影响主流程。"""
    if not code_list:
        return pd.DataFrame()

    q = _build_fundamentals_query(code_list)
    if q is None:
        return pd.DataFrame()

    try:
        if date_str:
            return get_fundamentals(q, date=date_str)
        return get_fundamentals(q)
    except Exception as e:
        print(f"[enrich_missing_fields_from_jq] get_fundamentals 失败（跳过）: {e}")
        return pd.DataFrame()


def enrich_missing_fields_from_jq(df):
    """
    对 CSV 中未提供或为空的字段调用 JoinQuant API 补齐。

    规则：
    1. CSV 已有且非空的值不覆盖；
    2. 行情字段优先用涨停日 zt_date 的日线数据补齐；
    3. latest_price / 当前名称等可用 current_data 或最近日线补齐；
    4. 估值/基本面字段用 get_fundamentals 批量补齐；
    5. JQ 不可用或单只股票失败时跳过，不中断整体分析。
    """
    if not CONFIG.get('enable_jq_field_fill', True):
        print('[enrich_missing_fields_from_jq] 已关闭 JoinQuant 字段补全')
        return df

    if not JQ_AVAILABLE:
        print('[enrich_missing_fields_from_jq] JQ 不可用，跳过字段补全')
        return df

    if 'jq_code' not in df.columns:
        print('[enrich_missing_fields_from_jq] 缺少 jq_code，跳过字段补全')
        return df

    df = df.copy()
    fill_count = 0

    # 确保后续会用到的标准列存在，哪怕 CSV 没提供这些列。
    fill_columns = set(PRICE_FIELD_MAP_JQ.keys()) | set(FUNDAMENTAL_FIELD_CANDIDATES.keys())
    fill_columns |= {'name', 'pct_change', 'price_change', 'amplitude', 'turnover_rate', 'avg_price'}
    df = _ensure_columns(df, fill_columns)

    print('[enrich_missing_fields_from_jq] 开始补齐 CSV 缺失字段...')

    # 1) current_data：补齐名称、最新价等轻量字段
    try:
        current_data = get_current_data()
    except Exception as e:
        current_data = None
        print(f"[enrich_missing_fields_from_jq] get_current_data 失败（跳过当前行情补齐）: {e}")
    ...
    if current_data is not None:
        for idx, row in df.iterrows():
            ...
            if not jq_code:
                continue
            try:
                cd = current_data[jq_code]
                for attr, col in [('name', 'name'), ('last_price', 'latest_price')]:
                    if hasattr(cd, attr):
                        if _fill_if_missing(df, idx, col, getattr(cd, attr)):
                            fill_count += 1
            except Exception:
                continue

    # 2) 涨停日/自选日的日线数据：补齐 open/high/low/pre_close/zt_close/volume/amount 等
    for idx, row in df.iterrows():
        jq_code = row.get('jq_code', '')
        ...
        date_str = zt_date.strftime('%Y-%m-%d') if isinstance(zt_date, pd.Timestamp) else str(zt_date)[:10]
        try:
            used_code, day = _get_single_day_price_safe(jq_code, date_str)
            if day is None:
                continue
            # 如果候选后缀修正成功，更新 jq_code，避免后续重复失败。
            if used_code and used_code != jq_code:
                df.at[idx, 'jq_code'] = used_code

            for local_col, jq_col in PRICE_FIELD_MAP_JQ.items():
                if jq_col in day.index:
                    if _fill_if_missing(df, idx, local_col, day[jq_col]):
                        fill_count += 1

            # 根据日线补齐涨跌、涨幅、振幅、均价等衍生字段。
            close_v = day.get('close', np.nan)
            pre_close_v = day.get('pre_close', np.nan)
            high_v = day.get('high', np.nan)
            low_v = day.get('low', np.nan)
            amount_v = day.get('money', np.nan)
            volume_v = day.get('volume', np.nan)

            if not _is_missing_value(close_v) and not _is_missing_value(pre_close_v) and pre_close_v:
                if _fill_if_missing(df, idx, 'price_change', close_v - pre_close_v):
                    fill_count += 1
                if _fill_if_missing(df, idx, 'pct_change', (close_v - pre_close_v) / pre_close_v * 100):
                    fill_count += 1
            if not _is_missing_value(high_v) and not _is_missing_value(low_v) and not _is_missing_value(pre_close_v) and pre_close_v:
                if _fill_if_missing(df, idx, 'amplitude', (high_v - low_v) / pre_close_v * 100):
                    fill_count += 1
            if not _is_missing_value(amount_v) and not _is_missing_value(volume_v) and volume_v:
                # A 股 volume 通常为股数；若环境返回手数，这里只作为辅助均价，不参与核心收益计算。
                if _fill_if_missing(df, idx, 'avg_price', amount_v / volume_v):
                    fill_count += 1

        except Exception as e:
            print(f"[enrich_missing_fields_from_jq] {jq_code} 日线字段补齐失败: {e}")
            continue

    # 3) 估值/基本面字段：批量补齐
    code_list = df['jq_code'].dropna().astype(str).unique().tolist()
    # 用最近可用基本面。也可以改为 date=zt_date，但逐日查询会慢很多。
    fundamentals_df = _get_fundamentals_safe(code_list)

    if fundamentals_df is not None and not fundamentals_df.empty and 'code' in fundamentals_df.columns:
        fundamentals_df = fundamentals_df.drop_duplicates(subset=['code']).set_index('code')
        for idx, row in df.iterrows():
            code = row.get('jq_code', '')
            if code not in fundamentals_df.index:
                continue
            frow = fundamentals_df.loc[code]
            for local_col, candidates in FUNDAMENTAL_FIELD_CANDIDATES.items():
                for src_col in candidates:
                    if src_col in fundamentals_df.columns:
                        if _fill_if_missing(df, idx, local_col, frow[src_col]):
                            fill_count += 1
                        break

    # 4) 二次数值清理，确保补齐后的字段类型可参与评分
    pct_cols = ['pct_change', 'amplitude', 'turnover_rate', 'revenue_yoy', 'profit_yoy', 'gross_margin']
    for col in pct_cols:
        if col in df.columns:
            df[col] = _clean_numeric_series(df[col])

    numeric_cols = list(fill_columns - {'name'})
    for col in numeric_cols:
        if col in df.columns:
            df[col] = _clean_numeric_series(df[col])

    print(f'[enrich_missing_fields_from_jq] 字段补齐完成，共补齐 {fill_count} 个单元格')
    return df
...
def get_price_data(cleaned_df, n_days=5):
    ...
    for idx, row in df.iterrows():
        ...
        try:
            # 获取涨停日及后 N 天的数据。
            # 注意：JoinQuant 的 get_price(panel=False) 在不同环境里可能没有 'time' 列，
            # 日期会放在 index；同时原版本用 end_date=zt_date 只能取到涨停日前数据，
            # 无法真正分析“涨停后 N 日”。这里统一改为 start_date=zt_date, end_date=zt_date+宽窗口。
            used_code, price_df = _get_price_forward_compatible(jq_code, zt_date_str, n_days)
            ...
            if len(after_zt) > 1:
                ...
                try:
                    ...
                    ma_df = _ensure_time_column(ma_df)
...
def run_analysis(file_path=None, filter_st=True, filter_yizhi=True):
    ...
    # Step 2.5: 对 CSV 未提供/为空的字段调用 JoinQuant API 补齐
    # CSV 中已有有效值不会被覆盖。
    cleaned_df = enrich_missing_fields_from_jq(cleaned_df)
