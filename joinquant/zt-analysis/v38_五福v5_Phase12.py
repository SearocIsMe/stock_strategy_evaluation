# 克隆自聚宽文章：https://www.joinquant.com/post/71748
# 标题：五福v5-人已疯魔版-11年385倍
# 作者：Heyibigboss
...
def initialize(context):
    ...
    log.info("【五福闹新春】v5.0启动！[DeepseekV4 v38: 风控放宽 -3%→-5%]")
...
#行业ETF：
    ...
    g.loss = 0.95  # v38: 从0.97放宽到0.95 (-3%→-5%)
...
def calculate_all_metrics_for_etf(etf, etf_name, hist_closes, hist_volumes, current_price, today_vol, context):
    try:
        ...
        if len(price_series) >= 10:
            try:
                laplace_s = g.laplace_s_param_weak if g.is_a_share_weak else g.laplace_s_param  # v10
...
def apply_filters(metrics_list):
    steps = [
        ...
        ('均线', lambda m: m['passed_ma'], False),  # v30: 全部关闭MA过滤
