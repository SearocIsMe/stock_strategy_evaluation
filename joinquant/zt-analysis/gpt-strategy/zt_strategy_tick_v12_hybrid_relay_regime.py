...
STRATEGY_CONFIG = {
    'strategy_version': 'v12_hybrid_relay_regime',
    # --- 股票池 ---
    ...
    # RELAY_HOT/RELAY_OK/HYBRID_HOT：允许核心打板；TREND_INDEX：禁用打板，只允许少量趋势低吸；ICE/RELAY_WEAK：空仓或极少交易。
    ...
    # --- v12 混合行情修正：指数趋势上涨但接力仍活跃时，不再完全禁用打板 ---
    'v12_hybrid_rawzt_threshold': 100,        # 昨日原始ZT>=100，说明短线仍有热度，进入HYBRID_HOT而不是纯TREND_INDEX
    'v12_hybrid_disable_top1_tick': False,    # HYBRID_HOT允许核心票tick打板
    'v12_relay_tick_candidate_n': 3,          # RELAY/HYBRID行情同时监听Top N核心tick，避免Top1不触板而错过Top2/Top3主升
    'v12_relay_tick_min_entry_index': 50,
    'v12_relay_tick_min_total_score': 28,
    'v12_relay_tick_hot_states': ['CORE_LEADER', 'HOT_CONTINUE'],
    'v12_hybrid_max_score_buys': 1,
...
def _make_entry_signal_from_row(row, entry_type: str, rank: int, signal_text: str) -> Dict:
    """v12：统一构造信号，避免多处字段遗漏。"""
    return {
        ...
        'second_leg_done': entry_type == 'SCORE_BUY',
    }


def generate_entry_signals(context, predict_df: pd.DataFrame) -> Dict:
    """
    v12：市场状态自适应 + 多核心tick监听。

    v11解决了“指数涨、策略跌”的反向问题，但在指数趋势与短线接力共振时过早禁用打板，
    容易错过一波主升。v12加入 HYBRID_HOT，并在 RELAY/HYBRID 下同时监听Top N核心票tick：
    - Top1不触板时，Top2/Top3仍可能成为当天真正主升龙头；
    - 只允许入选核心候选，不把逻辑改回乱买；
    - TREND_INDEX仍然保持低吸模式，ICE/RELAY_WEAK仍然空仓。
    """
    signals = {}
    ...
    if 'jq_code' not in df.columns:
        return signals
    for col in ['entry_index', 'total_score', 'v8_hot_score', 'v8_seen_count']:
        if col not in df.columns:
            df[col] = 0
        df[col] = pd.to_numeric(df[col], errors='coerce').fillna(0)
    for col in ['classification', 'signal', 'v8_hot_state']:
        if col not in df.columns:
            df[col] = ''
    ...
    # 不再过早 head(max_entry_count)。先保留更多候选，等09:31/ tick 二次过滤。
    scan_n = max(12, int(STRATEGY_CONFIG.get('hot_memory_keep_top_n_daily', 20) or 20))
    df = df.sort_values(['entry_index', 'total_score'], ascending=False).head(scan_n)
    ...
    if regime in ('ICE', 'RELAY_WEAK'):
        log.info(f"[generate_entry_signals] v12市场状态={regime}，停止新增信号")
        return signals

    # 纯指数趋势市：仍保持v11的低吸逻辑，避免再次与指数反向。
    if regime == 'TREND_INDEX':
        ...
        tdf = df[(df['total_score'] >= min_total) & (df['entry_index'] >= min_entry)].copy()
        if tdf.empty:
            log.info(f"[generate_entry_signals] v12趋势指数市无合格趋势低吸候选 total>={min_total}, entry>={min_entry}")
            return signals
        for rank, (_, row) in enumerate(tdf.head(STRATEGY_CONFIG.get('v11_trend_max_score_buys', 1)).iterrows(), start=1):
            code = row.get('jq_code', '')
            if code:
                signals[code] = _make_entry_signal_from_row(row, 'SCORE_BUY', rank, f'v12趋势指数市Top{rank}低吸')
        log.info(f"[generate_entry_signals] v12趋势指数市生成 {len(signals)} 个SCORE_BUY低吸信号，禁用Top1打板")
        return signals

    # RELAY/HYBRID：多核心tick监听，避免只盯Top1错过真正触板的主升票。
    tick_n = int(STRATEGY_CONFIG.get('v12_relay_tick_candidate_n', 3) or 3)
    tick_min_entry = float(STRATEGY_CONFIG.get('v12_relay_tick_min_entry_index', 50) or 50)
    tick_min_total = float(STRATEGY_CONFIG.get('v12_relay_tick_min_total_score', 28) or 28)
    hot_states = set(STRATEGY_CONFIG.get('v12_relay_tick_hot_states', ['CORE_LEADER', 'HOT_CONTINUE']) or [])

    tick_df = df[(df['entry_index'] >= tick_min_entry) & (df['total_score'] >= tick_min_total)].copy()
    if 'v8_hot_state' in tick_df.columns and hot_states:
        # 核心热点优先；若过滤后为空，则退回按entry_index前排，避免早期样本不足。
        core_df = tick_df[tick_df['v8_hot_state'].isin(hot_states)].copy()
        if not core_df.empty:
            tick_df = core_df
    tick_df = tick_df.sort_values(['entry_index', 'total_score', 'v8_hot_score'], ascending=False).head(tick_n)

    used = set()
    if not disable_top1:
        for rank, (_, row) in enumerate(tick_df.iterrows(), start=1):
            code = row.get('jq_code', '')
            if not code:
                continue
            signals[code] = _make_entry_signal_from_row(row, 'TOP1_TICK', rank, f'v12核心候选Top{rank} tick打板确认')
            used.add(code)

    # SCORE_BUY补充：从未进入tick监听的候选里挑，不再只看Top2~5固定名次。
    min_score_buy = STRATEGY_CONFIG.get('score_buy_min_entry_index', 45)
    score_slots = max(0, int(STRATEGY_CONFIG.get('max_entry_count', 3) or 3) - len(signals))
    score_df = df[(~df['jq_code'].isin(used)) & (df['entry_index'] >= min_score_buy)].copy()
    for i, (_, row) in enumerate(score_df.head(score_slots).iterrows(), start=1):
        code = row.get('jq_code', '')
        if code and code not in signals:
            signals[code] = _make_entry_signal_from_row(row, 'SCORE_BUY', len(signals) + 1, f'v12核心补位Top{i} 建仓指数买入')

    if signals:
        type_counts = {}
        for sig in signals.values():
            type_counts[sig.get('entry_type', 'UNKNOWN')] = type_counts.get(sig.get('entry_type', 'UNKNOWN'), 0) + 1
        log.info(f"[generate_entry_signals] v12生成 {len(signals)} 个建仓信号: {type_counts}; regime={regime}; tick监听候选={len(tick_df)}")
    else:
        log.info(f"[generate_entry_signals] v12无信号：regime={regime}, min_entry>={min_score_buy}")
    return signals


def setup_tick_subscriptions(context):
    """v12：订阅多个核心tick候选，而不是只盯Top1。"""
    if getattr(g, 'disable_top1_tick_today', False):
        ...
        g.top1_tick_codes = set()
        g.top1_tick_limits = {}
        log.info(f"[setup_tick_subscriptions] v12市场状态={getattr(g, 'v11_market_regime', 'UNKNOWN')}，禁用tick打板订阅")
        return
    ...
    tick_codes = []
    for code, sig in sorted(g.entry_signals.items(), key=lambda x: x[1].get('rank', 99)):
        if sig.get('entry_type') == 'TOP1_TICK':
            tick_codes.append(code)
    ...
    try:
        cur_data = get_current_data()
        for i, code in enumerate(tick_codes):
            try:
                high_limit = cur_data[code].high_limit
                if high_limit and high_limit > 0:
                    high_limit = float(high_limit)
                    g.top1_tick_codes.add(code)
                    g.top1_tick_limits[code] = high_limit
                    if i == 0:
                        g.top1_tick_code = code
                        g.top1_tick_limit = high_limit
                    subscribe(code, 'tick')
                    log.info(f"[setup_tick_subscriptions] v12订阅核心tick: {code}, high_limit={high_limit:.2f}")
            except Exception as e:
                log.info(f"[setup_tick_subscriptions] v12订阅tick失败: {code}, {e}")
    except Exception as e:
        log.info(f"[setup_tick_subscriptions] v12订阅tick总体异常: {e}")
...
def _update_market_regime_v11(context) -> None:
    ...
    # v12：指数趋势上涨不等于短线接力一定失效。
    # 若昨日原始涨停数仍 >=100，说明短线热度仍在，进入 HYBRID_HOT：
    # 既承认指数趋势市风险，又保留少量核心打板/主升机会，避免错过1月中旬这类短线爆发。
    hybrid_raw = int(STRATEGY_CONFIG.get('v12_hybrid_rawzt_threshold', 100) or 100)

    if emotion == 'WEAK' or idx_down:
        regime = 'ICE'
    elif relay_bad:
        regime = 'RELAY_WEAK'
    elif idx_up and raw_count >= hybrid_raw:
        regime = 'HYBRID_HOT'
    elif idx_up and raw_count < hybrid_raw:
        regime = 'TREND_INDEX'
    ...
    elif regime == 'HYBRID_HOT':
        # 指数趋势 + 短线热度并存：允许核心tick打板，但限制普通SCORE_BUY数量，避免全面追高。
        g.disable_top1_tick_today = bool(STRATEGY_CONFIG.get('v12_hybrid_disable_top1_tick', False))
        g.max_score_buys_today = min(int(getattr(g, 'max_score_buys_today', 1) or 1), int(STRATEGY_CONFIG.get('v12_hybrid_max_score_buys', 1) or 1))
        g.dynamic_score_buy_min_entry_index = max(float(getattr(g, 'dynamic_score_buy_min_entry_index', 50) or 50), 50.0)
        g.dynamic_score_buy_min_return = min(float(getattr(g, 'dynamic_score_buy_min_return', 0.008) or 0.008), 0.008)
        g.allow_new_entries_today = True
...
def initialize(context):
    ...
    log.info("[initialize] 涨停板交易策略 v12 初始化完成")
...
def handle_tick(context, tick):
    ...
    tick_codes = getattr(g, 'top1_tick_codes', None)
    if tick_codes:
        if not code or code not in tick_codes:
            return
    else:
    ...
    try:
        ...
        high_limit = None
        try:
            high_limit = getattr(g, 'top1_tick_limits', {}).get(code)
        except Exception:
            high_limit = None
