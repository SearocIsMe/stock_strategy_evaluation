...
STRATEGY_CONFIG = {
    ...
    'score_buy_min_open_strength': 0.0015,       # 09:31当前价至少强于开盘0.15%
    'score_buy_weak_reversal_open_strength': 0.008, # 若涨幅偏弱，必须强于开盘0.8%才允许低位反包
    'score_buy_loss_cooldown_days': 3,           # 最近3个自然日内亏损卖出的股票暂停再次买入
    'score_buy_low_quality_entry_index': 50,     # total_score偏低时，要求更高entry_index
    'score_buy_low_quality_total_score': 30,     # total_score低于30视为低质量候选
    'top1_reseal_mode': True,                    # Top1从单纯涨停价排板，改为“触板-开板-回封”优先
    ...
    'emotion_stop_zt_threshold': 40,       # v6.7: 极弱市才停止新增买入；40~89转为控仓而非禁买
    ...
    'emotion_caution_min_entry_index': 49,
    'emotion_normal_min_entry_index': 47,
    'emotion_hot_min_entry_index': 43,
    'emotion_caution_min_return': 0.0,    # v6.7: 谨慎市仍可进攻，但仓位/数量受控
    'emotion_normal_min_return': 0.003,
    'emotion_hot_min_return': 0.01,

    # --- v6.3 风控与交易闭环 ---
    'risk_check_times': ['09:35', '10:30', '11:25', '14:00', '14:50'],
    'profit_protect_min_pct': 0.035,       # v6.7: 浮盈超过3%后启用利润保护
    'profit_protect_drawdown_pct': 0.025,  # v6.7: 从最高价回撤2%则止盈
    ...
    'trailing_stop_pct': 0.06,      # 泛化移动止盈阈值；v6.7主要使用3%浮盈/2%回撤保护

    # --- 止损 ---
    'daily_stop_loss_pct': 0.05,   # v6.7：硬止损放宽到4.5%，避免涨停接力被洗出
    'structure_stop_enabled': True,  # v6.7：结构止损，跌破开盘且亏损时退出
...
def log_daily_summary(context) -> None:
    ...
    try:
        ...
        log.info(f"[log_daily_summary] v6.7交易统计输出异常: {e}")
...
def _log_trade_stats_v63(context) -> None:
    ...
    if not sells:
        log.info(f"📊 v6.7交易统计: 买入{len(buys)}笔，暂无已平仓交易，胜率/盈亏比待形成闭环")
        return
    ...
    # v6.7 自动诊断：用于快速判断策略是否假运行/过度交易/交易质量下降。
    try:
        all_signals = len(getattr(g, 'entry_signals', {}) or {})
        open_n = len(getattr(g, 'open_trades', {}) or {})
        if len(sells) >= 10:
            if win_rate < 0.45:
                log.info("🧠 [v6.7诊断] 胜率偏低：建议检查09:31买入质量过滤或降低弱市交易数量")
            if not pd.isna(profit_factor) and profit_factor < 1.3:
                log.info("🧠 [v6.7诊断] 盈亏比偏低：盈利保护/止盈延展可能不足")
        log.info(f"🧠 [v6.7诊断] 信号{all_signals} | 累计BUY{len(buys)} | SELL{len(sells)} | 未平仓{open_n}")
    except Exception as e:
        log.info(f"🧠 [v6.7诊断异常] {e}")
...
def initialize(context):
    ...
    log.info("[initialize] 涨停板交易策略 v6.7 初始化完成")
...
def before_trading_start(context):
    ...
    if g.zt_count_raw <= STRATEGY_CONFIG['zt_count_threshold']:
        ...
        g.trade_enabled_today = False
        log.info(f"[before_trading_start] ⚠️ v6.7情绪过滤：原始昨日ZT数 {g.zt_count_raw} < {STRATEGY_CONFIG.get('emotion_stop_zt_threshold', 60)}，今日不新增买入，只做持仓风控")
    else:
    ...
    try:
        ...
        for rec in reversed(getattr(g, 'trade_records', [])):
            ...
            if 0 <= delta_days <= days:
                log.info(f"[v6.7冷却跳过] {code} 最近{delta_days}天亏损卖出过，暂停买入")
                return True
            return False
    except Exception:
        return False
...
def _score_buy_quality_ok(context, code: str, sig: Dict, price: float, open_price: float, ret: float) -> Tuple[bool, str]:
    ...
    try:
        ...
        if open_strength < min_open_strength:
            return False, f"开盘后强度不足 open_strength={open_strength:.2%} < {min_open_strength:.2%}"
        ...
        if ret < 0.01 and open_strength < weak_reversal_strength:
            return False, f"弱涨幅缺少反包强度 ret={ret:.2%}, open_strength={open_strength:.2%}"
...
def buy_score_candidates(context):
    ...
    if max_score_buys <= 0:
        log.info(f"[SCORE_BUY_0931] v6.7情绪状态={getattr(g, 'market_emotion_state', 'UNKNOWN')}，Top2~5今日不新增买入")
        return
    ...
    for code, sig in signals:
        try:
            ...
            if bought_count >= max_score_buys:
                log.info(f"[SCORE_BUY_0931] v6.7情绪状态={getattr(g, 'market_emotion_state', 'UNKNOWN')}，已买满Top2~5上限 {max_score_buys} 只")
                break

            entry_index = float(sig.get('entry_index', 0) or 0)
            if entry_index < dyn_min_entry:
                log.info(f"[SCORE_BUY_0931跳过] {code} entry_index={entry_index:.1f} < v6.7动态阈值{dyn_min_entry:.1f}")
                continue
            ...
            # v6.7：二次质量过滤，避免v6.5放宽后买入过多弱质量票。
            quality_ok, quality_reason = _score_buy_quality_ok(context, code, sig, price, open_price, ret)
            if not quality_ok:
                log.info(f"[SCORE_BUY_0931跳过] {code} v6.7质量过滤: {quality_reason}")
                continue

            if ret < dyn_min_ret:
                log.info(f"[SCORE_BUY_0931跳过] {code} 09:31涨幅不足 ret={ret:.2%} < v6.7动态阈值{dyn_min_ret:.2%}")
                continue
    ...
    for code in list(g.holdings.keys()):
        ...
        if STRATEGY_CONFIG.get('structure_stop_enabled', True) and day_open > 0:
            if price < day_open and pnl_pct <= STRATEGY_CONFIG.get('structure_stop_min_loss_pct', -0.01):
                _sell_position(context, code, reason=f'固定风控-v6.7结构止损(跌破开盘且浮亏{pnl_pct:.2%})')
                continue
        if pnl_pct <= -STRATEGY_CONFIG.get('daily_stop_loss_pct', 0.045):
            _sell_position(context, code, reason=f'固定风控-v6.7硬止损({pnl_pct:.2%})')
            continue
        ...
        # 4. 利润保护 / 移动止盈：v6.7 只在曾经有足够浮盈时启用，避免“小浮盈后回落”被误判成移动止盈。
        if highest > buy_price:
            ...
            if max_profit_pct >= STRATEGY_CONFIG.get('profit_protect_min_pct', 0.04) and dd_from_high >= STRATEGY_CONFIG.get('profit_protect_drawdown_pct', 0.025):
                # v6.7：如果只是小浮盈后回落到亏损，先交给结构/硬止损处理；
                # 避免“利润保护”把大量小波动票变成SELL亏损记录。
                if pnl_pct > 0 or max_profit_pct >= STRATEGY_CONFIG.get('break_even_after_profit_pct', 0.05):
                    _sell_position(context, code, reason=f'固定风控-v6.7利润保护回撤({dd_from_high:.2%}, 最高浮盈{max_profit_pct:.2%})')
                    continue
            ...
            if max_profit_pct >= STRATEGY_CONFIG.get('break_even_after_profit_pct', 0.05) and pnl_pct <= STRATEGY_CONFIG.get('break_even_buffer_pct', -0.005):
                _sell_position(context, code, reason=f'固定风控-v6.7回本保护({pnl_pct:.2%}, 最高浮盈{max_profit_pct:.2%})')
                continue
            ...
            if max_profit_pct >= STRATEGY_CONFIG.get('profit_protect_min_pct', 0.04) and dd_from_high >= STRATEGY_CONFIG.get('trailing_stop_pct', 0.06):
                _sell_position(context, code, reason=f'固定风控-v6.7移动止盈回撤({dd_from_high:.2%})')
                continue
...
# v6.7 PATCH NOTES
# =========================
# 1. CAUTION状态只允许Top2建仓
# 2. Top1建议改为回封确认（需在tick逻辑中二次确认）
# 3. NORMAL/HOT状态建议提高仓位（需在order_value处扩展）
# 4. 盈利票延长持有（减少过早止盈）
