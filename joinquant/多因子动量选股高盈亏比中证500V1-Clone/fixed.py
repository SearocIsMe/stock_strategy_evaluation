...
def trade_routine(context):
    ...
    # 开仓逻辑
    if len(context.portfolio.positions) < 3:  # 最大持仓数
        log.info("开新仓！！！！")
...
def open_new_positions(context, day_idx):
    ...
    # 开仓
    stocks_to_buy = min(3 - len(context.portfolio.positions), len(results))  # 最多买5只
    
    if stocks_to_buy < 1:
        log.info(f"持仓数已满，不开新仓")
        return
...
def get_stock_pool_fixed(context):
    ...
    for stock in stocks:
        ...
        try:
            ...
            # 只过滤确实已知的信息
            if get_security_info(stock).start_date > prev_date:
                continue  # 新股排除
...
def calc_atr_from_prices_fixed(prices, period):
    ...
    # 计算ATR时排除最后一个数据点
    valid_tr = tr[:-1] if len(tr) > period else tr
    
    return np.mean(valid_tr[-period:]) if len(valid_tr) >= period else 0
