...
def check_limit_up_fixed(context):
    """安全版：检查昨日涨停股今日是否开板"""
    ...
    try:
        ...
        # yesterday_info 是一个 DataFrame，索引是日期，列是股票代码
        # 需要重构数据获取方式
        
        # 逐个股处理
        for stock in g.yesterday_HL_list:
            stock_data_current = current_data[stock]
            
            # 获取昨日收盘数据确认是否涨停
        if stock not in yesterday_info.columns:
            continue
            
        yesterday_close = yesterday_info[stock]['close']
        yesterday_high_limit = yesterday_info[stock]['high_limit']
        
        # 确认昨日确实涨停
        if abs(yesterday_close - yesterday_high_limit) < 0.01:
            # 今日使用开盘价判断是否开板
        ...
        today_open = today_data['open'].iloc[0]
            # 计算今日涨停价
            today_high_limit = round(yesterday_close * 1.10, 2)
            
        if today_open < today_high_limit:
            # 开盘即开板，执行卖出
            if not stock_data_current.paused:
                g.email_content += f"\n {stock}涨停打开，执行卖出"
        ...
        log.error(f"涨停监控出错: {e}")


def filter_stocks_fixed(context, stock_list):
    """安全版：使用历史数据过滤股票"""
    if not stock_list:
        return []
    ...
    for stock in stock_list:
        ...
        # 获取数据
        prev_open = prev_day_data['open'].iloc[0]
            prev_close = prev_day_data['close'].iloc[0]
            prev_high_limit = prev_day_data['high_limit'].iloc[0]
        prev_low_limit = prev_day_data['low_limit'].iloc[0]
        ...
        if current_data.is_st or '退' in current_data.name:
            continue
        ...
        if stock not in hold_list:
            # 前一日收盘涨停的剔除
            if abs(prev_close - prev_high_limit) < 0.01:
                continue
                
        if stock not in hold_list:
            # 前一日收盘跌停的剔除
            if abs(prev_close - prev_low_limit) < 0.01:
                continue
...
def get_stock_list(context):
    """从月度股票池筛选最终候选股票"""
    # 先进行基础过滤（剔除ST、新股等）
    filtered_stocks = filter_stocks_fixed(context, g.month_scope)
...
def trade_afternoon(context):
    ...
    if g.trading_signal:
        check_limit_up_fixed(context)   # 检查涨停股
...
def QQ_email_sending(subject,content,to_email):
    ...
    smtp_port = 587
    email_address = "jianghp2000@126.com"   # 发件人QQ邮箱
    password = "HP@750701"            ## 注意，授权码，非密码
